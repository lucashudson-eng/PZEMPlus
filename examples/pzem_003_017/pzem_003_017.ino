...
// Configure RS485 enable pin (MAX485)
#if defined(PZEM_RS485_EN)
    pzem.setEnable(PZEM_RS485_EN);
#endif

    // Set current range to 100, 50, 200 or 300A (PZEM-017 only)
...
void loop()
{
    ...
    startTime = millis();
    float highAlarmThreshold = pzem.getHighVoltageAlarm();
    uint32_t highAlarmThresholdTime = millis() - startTime;

    if (highAlarmThreshold >= 0)
    {
        Serial.print("High Voltage Alarm Threshold: ");
        Serial.print(highAlarmThreshold, 2);
        Serial.print(" V (");
        Serial.print(highAlarmThresholdTime);
        Serial.println("ms)");
    }
    else
    {
    ...
    startTime = millis();
    float lowAlarmThreshold = pzem.getLowVoltageAlarm();
    uint32_t lowAlarmThresholdTime = millis() - startTime;

    if (lowAlarmThreshold >= 0)
    {
        Serial.print("Low Voltage Alarm Threshold: ");
        Serial.print(lowAlarmThreshold, 2);
        Serial.print(" V (");
        Serial.print(lowAlarmThresholdTime);
        Serial.println("ms)");
    }
    else
    {
    ...
    if (pzem.readAll(&voltageAll, &currentAll, &powerAll, &energyAll))
    {
        ...
        // Calculate total time of individual methods (excluding alarms)
